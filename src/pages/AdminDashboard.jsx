    ...
    return (
        <main className="min-h-screen bg-[#F8FAFC] flex text-[#1E293B] font-sans">
            ...
            <aside 
                ...
                className={`bg-[#0F172A] text-slate-400 flex flex-col justify-between border-r border-slate-800 shrink-0 sticky top-0 h-screen z-40 transition-all duration-300 ${
                ...
                <div className="p-4 flex flex-col h-full justify-between overflow-hidden">
                    <div className="min-w-[240px]">
                        ...
                        <div className="flex items-center gap-3 mb-6 px-2">
                            ...
                            <div className={`transition-opacity duration-305 ${sidebarCollapsed ? "opacity-0" : "opacity-100"}`}>
                                <span className="text-sm font-black text-white block leading-tight tracking-tight">CloudPrint</span>
                                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">HQ Operations</span>
                            </div>
                        </div>
                        ...
                        <nav className="space-y-1 max-h-[calc(100vh-220px)] overflow-y-auto pr-1">
                            {[
                                { id: "queue", label: "🏠 Dashboard Overview", icon: "🏠" },
                                { id: "order-queue", label: "📋 Queue Kanban", icon: "📋" },
                                { id: "users", label: "👥 User Moderation", icon: "👥", onClick: fetchUsers },
                                { id: "frontend", label: "📊 Analytics", icon: "📊", onClick: () => { fetchSystemSettings(); fetchSections(); } },
                                { id: "system", label: "🖨️ Printer Settings", icon: "🖨️", onClick: () => { fetchSystemSettings(); fetchBlocks(); fetchPrinters(); } },
                                { id: "rewards", label: "📺 Display Panel", icon: "📺", onClick: () => { fetchRewards(); fetchSystemSettings(); } },
                                { id: "settings", label: "💰 Pricing & Coupons", icon: "💰", onClick: () => { fetchPrices(selectedPricingBlock); fetchCoupons(); fetchBlocks(); } },
                                { id: "blocks", label: "🏛️ Manage Blocks", icon: "🏛️", onClick: fetchBlocks },
                                { id: "support", label: "🎫 Support Tickets", icon: "🎫", onClick: fetchSupportTickets },
                                { id: "sql", label: "🗄️ SQL Terminal", icon: "🗄️", onClick: () => { setSqlResult(null); setSqlError(""); } }
                            ].map((item) => (
                                <button
                                    key={item.id}
                                    onClick={() => {
                                    ...
                                    className={`w-full flex items-center gap-4 px-3 py-2.5 rounded-xl text-[11px] font-black tracking-wider uppercase transition-all text-left ${
                                        activeTab === item.id
                    ...
                    <div className="p-2 bg-slate-950/40 border border-slate-800 rounded-xl flex items-center justify-between gap-3 mt-4 min-w-[240px]">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 text-white flex items-center justify-center font-black text-xs shadow-md shrink-0">
                                SA
                            </div>
                            <div className={`leading-tight text-left transition-opacity duration-300 ${sidebarCollapsed ? "opacity-0" : "opacity-100"}`}>
            ...
            <div className="flex-1 min-w-0 flex flex-col min-h-screen">
                ...
                <header className="sticky top-0 z-30 bg-white/80 border-b border-slate-200/80 backdrop-blur-md px-8 py-4 flex items-center justify-between gap-4 shadow-sm">
                    <div className="flex items-center gap-3">
                        <h2 className="text-xs font-black uppercase tracking-widest text-slate-400">HQ System Monitor</h2>
                    </div>
                ...
                <div className="p-8 flex-1">
                    ...
                    <section className="p-6 rounded-2xl bg-white border border-slate-200/80 shadow-sm mb-8 flex flex-wrap justify-between items-center gap-6">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900">Welcome Back, Administrator 👋</h2>
                            <p className="text-sm font-bold text-slate-400 mt-1">
                                Monitor every printer, every order, every student, and every transaction from one intelligent dashboard.
                            </p>
                        </div>
                        <div className="flex items-center gap-6">
